...
function isCreditCardNumber(ccn) {
  const str = ccn.toString().split('').map((el) => el * 1);
  let sum = 0;
...
function getCommonDirectoryPath(pathes) {
  const commDir = [];
  const arr = pathes.map((path) => path.split(/(\/)/)).sort((a, b) => b.length > a.length);
  const firstPath = arr.shift();
  for (let i = 0; i < firstPath.length; i += 1) {
    const path = firstPath[i];
    if (arr.every((path2) => path2[i] === path)) {
      commDir.push(path);
    } else break;
  }
  return commDir.join('');
...
function evaluateTicTacToePosition(position) {
  for (let row = 0; row < 3; row += 1) {
    if (
      position[row][0] && position[row][0] === position[row][1]
  ...
  for (let column = 0; column < 3; column += 1) {
    if (
      position[0][column] && position[0][column] === position[1][column]
